    ...
    if (isString(value) || isNumber(value)) {
      ...
      loop(value, function(item, index) {
        if(value.length < last.length){
          eom[key][index].element.parent.appendChild(ele.element.cloneNode(true));
        }else if(value.length > last.length){
          eom[key][index].element.parent.removeChildNode();
        }
        update(item, index, eom[key][index], last);
      });
    } else if (isObject(value)) {
      each(value, function(item, index) {
        update(item, index, eom, last);
      });
    }
  }
